...
public class CommentService {
	...
	@Transactional
	public DeleteCommentResponse deleteComment(DeleteCommentRequest deleteCommentRequest, Long id) {
		return commentRepository.findByFeed_IdAndId(id, deleteCommentRequest.id())
			.map(comment -> {
				comment.deleteComment(deleteCommentRequest);
				return DeleteCommentResponse.of(true);
			})
			.orElseThrow(() -> new NoSuchElementException("존재하지 않는 댓글입니다"));
	}
}
